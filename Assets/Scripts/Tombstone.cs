...
public class Tombstone : MonoBehaviour, IInteractable, IDiggable
{
    ...
    private int WhichRarityToDrop()
    {
        ...
        if ((rarityValue / currentRarityQuantity) > 0.25f) // common = 75% (1 - 0.25 = 0.25)
        {
            return 1;
        }
        else if ((rarityValue / currentRarityQuantity) > 0.65f) // rare = 35% (1 - 0.65 = 0.35)
        {
            currentRarityQuantity *= multiplierQuantity;
            return 2;
        }
        else if ((rarityValue / currentRarityQuantity) > 0.85f) // supernatural = 15% (1 - 0.85 = 0.15)
        {
            currentRarityQuantity *= multiplierQuantity;
            return 3;
        }
    ...
    IEnumerator IInteractable.ShowInteractable()
    {
        if (quantityDropped < 3)
        {
            float journey = 0;
    ...
    IEnumerator IInteractable.HideInteractable()
    {
