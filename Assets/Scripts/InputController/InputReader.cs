...
public class InputReader : ScriptableObject, CharacterControls.IGameplayActions
{
	...
	private Vector2 startMousePosTest, targetMousePosTest;
	...
	public int OnDigTest(GameObject canvasToDig)
	{
		if (Mouse.current.leftButton.IsPressed()) //click mouse down (holding)
		{
			...
			if (uiRaycast == null)
				uiRaycast = canvasToDig.GetComponent<GraphicRaycaster>();

			uiRaycast.Raycast(clickData, clickRayResults);

			foreach(RaycastResult rr in clickRayResults)
			{
				if (rr.gameObject.TryGetComponent(out DigTest toDigTest))
				{
					return toDigTest.testPosition;
				}
			}
	...
	{
		startMousePosTest = Mouse.current.position.ReadValue();

		targetMousePosTest = startMousePosTest + multiplyPos;
	}

	public Vector2 GetOriginMousePosition()
	{
		return startMousePosTest;
	}
