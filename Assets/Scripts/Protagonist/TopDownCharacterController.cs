...
public class TopDownCharacterController : MonoBehaviour
{
    ...
    private void StartInteract()
    {
        ...
        if (hit != null)
        {
            ...
            {
                if(colCheck.TryGetComponent(out IDiggable toDig) && !toDig.hasBeenDug)
                {
                    TryToDig(colCheck.gameObject);
                    toDig.hatLuck = currentHatLuck;
                }
    ...
    private void DigTestChecking()
    {
        if (IsHoldingDig) //it needs to hold
        {
            if (inputReader.OnDigTest(digTestCanvas) == 1)
            {
                ...
                {
                    inputReader.HideCursor();
                    inputReader.SetDigMousePosition(stage1To2Multiplier);
                    currentTestPosition = 1;
                    //new GameObject("targetPosition 1").transform.position = inputReader.GetTargetMousePosition();
                }
            }
            ...
            if (currentTestPosition == 1)
            {
                if (fillStage1to2.fillAmount >= 1)
                {
                    ...
                    //new GameObject("targetPosition 2").transform.position = inputReader.GetTargetMousePosition();
                }
                else
                {
